...
const spreadsheetId = '1IcTnDh4UiHKBUqzfEWOW4f95bta9rv2QSv2IltdXwLk';
const range = '開拓リスト（緯度経度）!B:F';
const apiKey = 'AIzaSyDqh5zuCekqccxBqo9P48SIvPVVoUHl8uw'; // HTMLで設定されたGoogle Maps APIキー
...
async function fetchHospitals() {
    try {
        ...
        if (data.values) {
            // スプレッドシートのデータを病院オブジェクトの配列に変換
            return data.values.map(row => ({
                name: row[0],           // 病院名
                specialties: row[1],    // 診療科
                address: row[2],        // 住所
                lat: parseFloat(row[3]), // 緯度（文字列→数値変換）
                lng: parseFloat(row[4])  // 経度（文字列→数値変換）
            }));
        } else {
...
function processResults(response, maxTime, hospitals) {
    ...
    if (response.rows[0] && response.rows[0].elements) {
        response.rows[0].elements.forEach((element, index) => {
            if (element.status === "OK" && element.duration.value <= maxTimeInSeconds) {
